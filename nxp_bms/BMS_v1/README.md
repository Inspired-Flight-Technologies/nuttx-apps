...
```bash
(cd apps; git checkout releases/11.0)
```
...
Copy and unzip the BMS files in that folder
```bash
cd apps/nxp_bms/BMS_v1
cp <location BMS5.17_dronecan.zip> ./
unzip BMS5.17_dronecan.zip
cd ../..
```
...
```bash
(cd apps; patch -p1 < nxp_bms/BMS_v1/Patchfiles/0001-apps-patch-BMS5.17.patch)
```
...
```
```bash
git checkout releases/11.0
```
Add the nuttx patch.
```bash
patch -p1 < ../apps/nxp_bms/BMS_v1/Patchfiles/0001-nuttx-patch-BMS54.17.patch
```
